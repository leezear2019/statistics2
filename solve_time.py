...
if __name__ == '__main__':
    ...
    # summary_files = sorted(os.listdir(root_dir))
    # 保留列
    solve_time_list = ['instance', 'time', 'filterTime', 'time.1', 'filterTime.1', 'time.2', 'filterTime.2', 'time.3',
                       'filterTime.3', 'time.4', 'filterTime.4']
    # 新标题列
    new_title = ['instance', 'Regin-solve', 'Regin-filter', 'Zhang18-solve', 'Zhang18-filter', 'Zhang20-solve',
                 'Zhang20-filter', 'WordRam-solve', 'WordRam-filter', 'Our-solve', 'Our-filter']
    print(solve_time_list)
    print(new_title)
    # summary_files = sorted(os.listdir(root_dir))
    #
    ...
    # data = pd.read_csv("D:\exp\large\Partition.csv")
    # data = pd.read_csv("D:\exp\large\GolombRuler.csv")
    # data = pd.read_csv("D:\exp\large\AllInterval.csv")
    # 只保留求解时间的列
    # 删nan的行
    data = data[solve_time_list]
    data = data.dropna(axis=0, how='any')
    ...
    c2 = data[new_title[3]] >= time_limit
    c3 = data[new_title[5]] >= time_limit
    c4 = data[new_title[7]] >= time_limit
    c5 = data[new_title[9]] >= time_limit
    # c6 = data[new_title[5]] >= time_limit
    # c7 = data[new_title[6]] >= time_limit
    # c8 = data[new_title[7]] >= time_limit
    # data.drop(data[c1|c2|c3|c4|c5|c6].index, inplace=True)
    ...
    c2 = data[new_title[3]] < min_time
    c3 = data[new_title[5]] < min_time
    c4 = data[new_title[7]] < min_time
    c5 = data[new_title[9]] < min_time
    # c6 = data[new_title[5]] < min_time
    # c7 = data[new_title[6]] < min_time
    # c8 = data[new_title[7]] < min_time
    ...
    # data.to_csv("D:\exp\large\Solve_AllInterval2.csv", index=0)
    data.to_csv("D:\exp\large\Solve_CostasArrays2.csv", index=0)
    # data.to_csv("D:\exp\large\Solve_MagicSquare2.csv", index=0)
    # data.to_csv("D:\exp\large\Solve_Partition2.csv", index=0)
    # data.to_csv("D:\exp\large\Solve_GolombRuler2.csv", index=0)
