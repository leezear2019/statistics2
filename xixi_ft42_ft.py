...
if __name__ == '__main__':
    ...
    max_time = 899.9
    min_time = 0.001
    algos = ['WordRam', 'Zhang18', 'Zhang20', 'Our']
    ...
    # 求解时间
    solve_time_list = ['time', 'time.1', 'time.2', 'time.3', 'time.6']
    # solve_time_list = ['time.1','time.1', 'time.2', 'time.3', 'time.6']
    # 过滤时间
    filterTime_list = ['filterTime', 'filterTime.1', 'filterTime.2', 'filterTime.3', 'filterTime.6']
    ...
    new_title = ['Regin', 'WordRam', 'Zhang18', 'Zhang20', 'Our']
    ...
    f, ax = plt.subplots(3, 1, sharex=True, sharey=True, figsize=(4, 10))
    # plt.rcParams['font.sans-serif'] = ['Times New Roman']
    f.subplots_adjust()
    plt.ylim(0.001, 200)
    plt.xlim(0.001, 200)
    ...
    for i in range(3):
        algo = ts[i]
        ax[i].scatter(ds[i][algo[0]], ds[i][algo[1]], s=0.25)
