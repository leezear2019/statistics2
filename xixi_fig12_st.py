...
if __name__ == '__main__':
    ...
    # 新标题列
    # new_title = ['Régin', 'Zhang18', 'Our$^M$', 'Zhang20', 'WordRam', 'Our$^{MB}$', 'LO']
    new_title = ['WordRam', 'Zhang18', 'Zhang20', 'Our']
    ...
    for name in summary_files:
        ...
        data.drop(data[c1 & c2 & c3 & c4].index, inplace=True)
    ...
    # summary_data = summary_data.drop('Our$^M$', 1)
    # # new_title.remove('Choco')
    # new_title.remove('Our$^M$')
    ...
    for i in range(size):
        ...
        #     continue
        # else:
        plt.plot(summary_data[new_title[i]],  # x轴数据
                 summary_data.index,
                 label=new_title[i], linewidth=0.7)  # 添加标签
        # plt.set_ylabel("#Solved instances",fontsize=14)
        # plt.set_xlabel("CPU time (seconds)",fontsize=14)
    plt.semilogx()
    plt.legend()
    plt.show()
...
#
#     # plt.legend()
#     # plt.legend(loc=3, bbox_to_anchor=(-1.45, 2.3), borderaxespad=0., ncol=3,fontsize=12,facecolor='none')  ##设置ax4中legend的位置，将其放在图外
#     # plt.legend(loc=3, bbox_to_anchor=(-0.2, 1.17),fontsize=12,facecolor='none')  ##设置ax4中legend的位置，将其放在图外
